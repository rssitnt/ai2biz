    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-28 right-10 z-50 sm:bottom-20 sm:right-5 md:bottom-28 md:right-10">
          <div 
            className="w-80 sm:w-80 md:w-96 card overflow-hidden glassmorphism backdrop-blur-md max-w-[calc(100vw-20px)] relative before:absolute before:inset-0 before:bg-gradient-to-r before:from-blue-500/30 before:via-purple-500/30 before:to-indigo-500/30 before:animate-gradient-x before:-z-10 shadow-xl"
            style={{ 
              borderRadius: '26px',
              border: '2px solid transparent',
              backgroundImage: 'linear-gradient(rgba(17, 17, 17, 1), rgba(17, 17, 17, 1)), linear-gradient(to right, #3b82f6, #a855f7, #6366f1)',
                    ...
                    {chatType === 'site' ? (
                      <div className="relative group">
                        <button 
                          ...
                          className="flex items-center justify-center rounded-full transition-all duration-300 hover:opacity-80 select-none"
                          aria-label="Перейти в Телеграм"
                        >
                          <Image 
                            src={`/telegram.png?v=${Date.now()}`}
                            alt="Телеграм" 
                            width={32} 
                            height={32} 
                        ...
                        <span className="absolute left-1/2 top-full mt-2 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-pre bg-zinc-800/90 py-1 px-2 rounded pointer-events-none transform -translate-x-1/2 text-center">Перейти в<br/>Телеграм</span>
                      </div>
                    ) : (
                      <button 
                        onClick={toggleChatType}
                        className="flex items-center gap-2 py-2 px-4 rounded-[20px] transition-all duration-300 hover:bg-zinc-800/70 select-none group"
                      >
                        <BsChatSquareTextFill className="text-2xl text-blue-400" />
                        <span className="text-sm text-zinc-300 font-medium">Вернуться в чат</span>
                      </button>
                    )}
                  </>
                )}
                
                {hasChosenChatType && chatType === 'site' && (
                  <div 
                    className="relative flex p-1 bg-[#1C1C1E]/90 rounded-full shadow-sm cursor-pointer select-none group"
                  ...
                  >
                    ...
                    {/* Всплывающая подпись */}
                    <span className="absolute left-1/2 top-full mt-2 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-pre bg-zinc-800/90 py-1 px-2 rounded pointer-events-none transform -translate-x-1/2 text-center">Переписка<br/>или аудио</span>
                  </div>
                )}
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="flex flex-col space-y-3 mb-4">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 py-3 px-4 rounded-[20px] text-sm bg-zinc-800 hover:bg-zinc-700 text-white transition-all duration-200 select-none"
                    >
                      <BsChatSquareTextFill className="text-3xl text-blue-400" />
                      <span className="font-medium">Чат на сайте</span>
                    </button>
                    
                    <div 
                      className="flex items-center justify-center gap-2 py-3 px-4 rounded-[20px] text-sm bg-zinc-800 hover:bg-zinc-700 text-white transition-all duration-200 cursor-pointer"
                      onClick={() => handleChatTypeChoice('telegram')}
                    >
                      <button
                        className="transition-all duration-200 hover:opacity-80 select-none"
                        aria-label="Продолжить общение в Telegram"
                      >
                        <Image 
                          src={`/telegram.png?v=${Date.now()}`}
                          alt="Телеграм" 
                      ...
                      <span className="font-medium text-white">Телеграм</span>
                    </div>
                  </div>
                </>
              )}
              
              {chatType === 'site' && hasChosenChatType && (
                <div>
                  <div 
                    ref={chatContainerRef}
                    className="p-3 mb-8 h-40 sm:h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent"
                    style={{ borderRadius: '20px' }}
                  >
                    {messages.map((message, index) => (
                      <div key={index} className={`flex items-start mb-3 ${message.sender === 'user' ? 'justify-end' : ''}`}>
                        <div 
                          className={`p-2 text-sm max-w-[85%] ${
                            message.sender === 'user' 
                  ...
                  <div className="min-h-[52px] flex items-start relative pb-10">
                    {/* Условный рендеринг: для текстового режима показываем поле ввода, для голосового - микрофон */}
                    {inputMode === 'text' ? (
                      // Текстовый режим
                      <div className="w-full flex items-start relative justify-center mt-6">
                        ...
                        >
                          <textarea 
                            ...
                            className="w-full pr-[80px] glassmorphism py-2 pl-5 placeholder-opacity-50 focus:outline-none focus:ring-1 focus:ring-blue-500/30 bg-transparent text-white text-sm min-h-[42px] max-h-[120px] resize-none overflow-hidden"
                            rows={1}
                            style={{ height: '42px', lineHeight: '1.2', borderRadius: '30px', paddingTop: '12px' }}
                          />
                          <div className="absolute right-3 top-1/2 flex gap-1 items-center transform -translate-y-1/2">
                            <div className="relative group">
                              <button 
                              ...
                              <span className="absolute left-1/2 -top-8 mt-2 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-pre bg-zinc-800/90 py-1 px-2 rounded pointer-events-none transform -translate-x-1/2 text-center">Голосовой<br/>ввод</span>
                            </div>
                            <div className="relative group">
                              <button 
                              ...
                              <span className="absolute left-1/2 -top-8 mt-2 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-pre bg-zinc-800/90 py-1 px-2 rounded pointer-events-none transform -translate-x-1/2 text-center">Отправить<br/>сообщение</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      // Голосовой режим
                      <div className="w-full flex items-center justify-center min-h-[44px] mt-6 py-2">
                        <div className="relative group">
                          <button 
                          ...
                          <span className="absolute left-1/2 -top-8 mt-2 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-pre bg-zinc-800/90 py-1 px-2 rounded pointer-events-none transform -translate-x-1/2 text-center">Запись<br/>голоса</span>
                        </div>
